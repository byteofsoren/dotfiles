...
# linkconffiles creates symlinks in the .vim/after directory.
# $ linkconffiles sourec installdir
#
...
function install() {
	...
	# linkconffiles [local source dir] [taregt dir]
	linkconffiles ftplugin ftplugin
	linkconffiles bytevim plugin
	copykeysfile
	...
	# VimWiki settings need a its own symlink and wold not work with out.
	vimwiki_conf="vimwiki_conf.vim"
	vimwiki_target_dir="$HOME/.vim/plugin/vimwiki_settings"
	vimwiki_target_conf_file="$vimwiki_target_dir/$vimwiki_conf"
	# if [[ ! -L "$HOME/.vim/plugin/vimwiki_settings/vimwiki_conf.vim" ]] && [[ ! -e "$HOME/.vim/plugin/vimwiki_settings/vimwiki_conf.vim" ]]; then
	if [[ ! -L $vimwiki_target_conf_file ]] && [[ ! -e $vimwiki_target_conf_file ]]; then
		mkdir -p "$vimwiki_target_dir"
		# ln -s "$(pwd)/$vimwiki_conf" "$HOME/.vim/plugin/vimwiki_settings/vimwiki_conf.vim"
		ln -s "$(pwd)/$vimwiki_conf" "$vimwiki_target_conf_file"
	fi
...
function clean() {
	...
	if confirm "Do you want to unlink the vimrc?"; then
		unlink "$HOME/.vimrc"
		unlink "$HOME/.vim/plugin/vimwiki_settings/vimwiki_conf.vim"
	fi
